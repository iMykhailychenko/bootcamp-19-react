...
const useMyHook = () => {
  ...
  const handleClose = () => setIsSpecialOfferOpen(false);
  const handleOpen = () => setIsSpecialOfferOpen(true);

  return { isSpecialOfferOpen, handleClose, handleOpen };
};
...
export const MarketingInfo = () => {
  ...
  const { isSpecialOfferOpen, handleClose, handleOpen } = useMyHook();

  useEffect(() => {
    if (divElement.current) {
      console.dir(divElement.current);
    }
  });
  ...
  return (
    <>
      <div ref={divElement} className="mt-2 mb-5">
